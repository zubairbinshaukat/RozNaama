...
export default function AddExpenseModal({ onClose }: AddExpenseModalProps) {
  ...
  const validate = (): { ok: boolean; message?: string } => {
    ...
    if (isNaN(v) || v <= 0) return { ok: false, message: 'Enter a valid expense amount greater than 0.' }
    return { ok: true }
  }
        ...
        >
          ...
          <div className="flex items-center gap-3 px-5 pt-5 pb-4 shrink-0">
            <div className="w-10 h-10 rounded-2xl bg-destructive/10 flex items-center justify-center shrink-0">
              <Receipt size={18} className="text-destructive" strokeWidth={1.8} />
            </div>
            <div className="flex-1 min-w-0">
              <h2 id="add-expense-title" className="text-base font-bold text-foreground tracking-tight">
                Add Expense
              </h2>
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="expense-note" className="text-sm font-semibold text-foreground">
              ...
              {!isNaN(parsedAmount) && parsedAmount > 0 && (
                <p className="text-xs text-muted-foreground">
                  Expense total:{' '}
                  <span className="font-semibold text-destructive tabular-nums">{formatCurrency(parsedAmount)}</span>
                </p>
              )}
            </div>

            <div className="px-5 pt-4 shrink-0 flex items-center justify-between gap-4" style={{ borderTop: '1px solid color-mix(in srgb, var(--border) 50%, transparent)' }}>
              <div className="text-sm font-semibold text-foreground">
                Today total will update:{' '}
                <span className="text-primary text-base font-bold tabular-nums">Sales - Expenses</span>
              </div>
